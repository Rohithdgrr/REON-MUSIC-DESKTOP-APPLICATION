<template>
  <div class="track-card" @click="handlePlay" :class="{ playing: isCurrentTrack, 'no-index': index === undefined }">
    <!-- Index Column -->
    ...
    <!-- Title Column -->
    <div class="title-col">
      <span class="title-text" :title="track.title">{{ track.title }}</span>
...
.track-card.no-index {
  grid-template-columns: 56px 2fr 1.2fr 140px 60px;
}
...
@media (max-width: 900px) {
  ...
  .track-card.no-index {
    grid-template-columns: 56px 1fr 140px 60px;
  }
